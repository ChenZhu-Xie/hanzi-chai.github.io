...
if ($null -ne $runningMode) {
    throw "Port $Port is running hanzi-chai in $runningMode mode. Stop it before starting PAGES mode."
}
...
Write-Host "Starting PAGES mode (HashRouter): $AdminUrl"
& $bun.Source start --host $HostAddress --port $Port --strictPort
if ($LASTEXITCODE -ne 0) {
    throw "bun start failed with exit code $LASTEXITCODE."
}
